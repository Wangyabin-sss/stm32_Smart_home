...
{
	int i=0;
	printf("mqtt.c-sever_bak=%d,%x,%x,%x\n",len,RX2_data.data[0],RX2_data.data[1],RX2_data.data[2]);
	for(i=0;i<len;i++)
	{
		if(RX2_data.data[i]==data[0]&&RX2_data.data[i+1]==data[1])
			return 0;
	}
	return 1;
}
...
static int find_usart_bak(char *data) //等待串口返回OK,返回值为0表示正常
{
	uint32_t time=0;
	while(time<200){
		if(strstr((char *)RX2_data.data,data)!=NULL)
			return 0;
		delay_ms(10);time++;
	}return 1;
}
...
int mqtt_connect(void)
{
	...
	usart2_clear();
	usart2_send_str(SEVERADDR);         //连接服务器
	...
	usart2_clear();
	usart2_send_str("AT+CIPMODE=1\r\n"); //开启透传模式
	if(find_usart_bak("OK"))
		return -2;
	
	usart2_clear();
	usart2_send_str("AT+CIPSEND\r\n");  //开启透传
	if(find_usart_bak("OK"))
		return -3;
	
	usart2_clear();
	usart2_send_hex(data,datalen);free(data);
	delay_ms(500);
	if(find_sever_bak((uint8_t *)sever_bak[0],RX2_data.a))
		return -4;
	return 0;
}
...
int mqtt_subtopic(char *sub)
{
	...
	usart2_send_hex(data,datalen);free(data);
	delay_ms(500);
	if(find_sever_bak((uint8_t *)sever_bak[2],RX2_data.a))
		return -1;
	return 0;
}
...
//		pubdata[1]=alllength;
//	}
//	usart2_send_hex(pubdata,pubdata[1]+3);
//	delay_ms(100);
//}
...
uint16_t mqtt_publish_data(char *topic, char *message, uint8_t qos)
{
    ...
    usart2_send_hex(g_esp8266_tx_buf,g_mqtt_tx_len);
	delay_ms(100);
...
{
	usart2_clear();
	usart2_send_hex((uint8_t*)tick,2);
	delay_ms(500);
	if(find_sever_bak((uint8_t *)sever_bak[1],RX2_data.a))
	{
		usart2_clear();
		return -1;
	}
...
{
	usart2_clear();
	usart2_send_str("AT+CIFSR\r\n");
	delay_ms(500);
	//printf("wifi_check:%s\n",RX2_data.data);
	if(strstr((char *)RX2_data.data,"\"0.0.0.0\"")==NULL)
		return 0;
	return -1;
}
